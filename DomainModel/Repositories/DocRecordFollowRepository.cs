...
{
    public class DocRecordFollowRepository : RepositoryBase<DocRecordFollow>
    {
        ...
        public string GetMaxDocFollowNo(string docId)
        {
            return Context.DocRecordFollows
                .Where(p => p.DocRecodId == docId)
                .Select(x => x.Id).Max();
        }
    }
